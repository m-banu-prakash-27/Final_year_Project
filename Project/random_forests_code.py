...
print('The training r_sq is: %.2f'% rfreg.score(x_train,y_train))

# Training model evaluation
from sklearn.metrics import mean_absolute_error, mean_squared_error, explained_variance_score, r2_score 

# Prediction on the DS
ytrain_pred=rfreg.predict(x_train)
...
ytest_pred=rfreg.predict(x_test)

# The r_sq
print('The testing r_sq is: %.2f'% r2_score(y_test,ytest_pred))

print('The testing r_sq is: %.2f'% rfreg.score(x_test,y_test))
...
print('Rank: %.0f'%rfreg.predict(new_data))
...
if len(rc)!=0:
    for i,r in rc.iterrows():
        ...
        nd.append(lst)
    nd
    print('Rank: %.0f'%rfreg.predict(nd))
else:
